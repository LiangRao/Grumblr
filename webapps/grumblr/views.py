...
def global_stream(request):
    context = {}
    context['posts'] = Post.get_posts_global_stream(request.user)
    context['user'] = request.user
    context['comment_redirect'] = 'stream'
    context['bloggers'] = User.objects.exclude(username=request.user.username)
...
def register(request):
    ...
    send_mail(subject="Verify your email address",
              message=email_body,
              from_email="hsuehlih@andrew.cmu.edu",
...
@login_required
def get_global_stream_posts(request, time="1970-01-01T00:00+00:00"):
    # user = User.objects.get(id=user_id)
    max_time = Post.get_max_time_global_stream(user=request.user)
    posts = Post.get_posts_global_stream(user=request.user, time=time)
    context = {"max_time": max_time, "posts": posts}
    return render(request, 'posts.json', context, content_type='application/json')
...
@login_required
def get_global_stream_changes(request, time="1970-01-01T00:00+00:00"):
    # user = User.objects.get(id=user_id)
    max_time = Post.get_max_time_global_stream(user=request.user)
    posts = Post.get_changes_global_stream(user=request.user, time=time)
    context = {"max_time": max_time, "posts": posts}
    return render(request, 'posts.json', context, content_type='application/json')
